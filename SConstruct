...
SRC_EXCLUDE   = ["testevaluation.cpp"]      # excluded file names
...
if TOOLSET == "gcc":
    ...
    if STRICT:
        DFT_CCFLAGS[0] += " -finput-charset=gbk -Werror=implicit-function-declaration -Werror=return-type -Werror=return-local-addr -Werror=uninitialized -Werror=delete-non-virtual-dtor"
    if PROFILE:
        DFT_CCFLAGS[1] += " -pg"
    if COVERAGE:
